 ...
 *
 */
export const MetricsTemplate = (): string => {
  return `
     ...
     */
    const scale = (size: number, isRounded: boolean = false): number =>
      isRounded ? Math.ceil(size * baseSize) : size * baseSize;
    ...
    const globalMetrics: GlobalMetricsType = {
      ...
      isLowerThenAndroid11: Platform.OS === 'android' && Platform.Version < 30
    };
